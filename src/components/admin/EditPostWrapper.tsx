...
export default function EditPostWrapper({ post }: { post: BlogPost }) {
    ...
    return (
        <div className="max-w-4xl mx-auto py-10 px-6">
            <Link href="/admin/dashboard/blog" className="inline-flex items-center gap-2 text-emerald-400 hover:text-emerald-300 transition-colors mb-10 font-bold">
                <ChevronLeft size={20} /> Back to Blog List
            </Link>
            ...
            <div className="mb-12">
                ...
                <p className="text-slate-400 text-lg">Update your insights and keep your content fresh.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-10 glass p-10 md:p-14 rounded-[3.5rem] border-slate-800 shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="space-y-6">
                        <div>
                            <label className="block text-sm font-bold text-slate-300 mb-3" htmlFor="slug">
                                URL Slug (Changing this will break old links!)
                            </label>
                            <input
                                id="slug"
                                name="slug"
                ...
                <div className="pt-8 border-t border-slate-800/50 text-right">
                    <button
                        type="submit"
                        disabled={isLoading || isUploading}
                        className="px-12 py-5 rounded-2xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold transition-all shadow-[0_0_40px_rgba(16,185,129,0.3)] hover:shadow-[0_0_50px_rgba(16,185,129,0.5)] inline-flex items-center gap-4 disabled:opacity-70 group"
                    >
                        {isLoading ? <Loader2 className="animate-spin" size={24} /> : (
                            <>
                                Update Article <Save size={24} className="group-hover:rotate-12 transition-transform" />
                            </>
                        )}
                    </button>
                </div>
